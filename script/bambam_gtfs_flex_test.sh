...
rust/target/release/bambam-omf network \
    -n minnesota-flex \
    -c configuration/bambam-omf/travel-mode-filter.json \
    -o "$WORKING_DIR" \
    -b -95.14435,-93.76282,44.01652,44.568947
...
rust/target/release/bambam_util gtfs-flex-config-app \
    --base-file "$WORKING_DIR"/bambam.toml \
    --out-file "$WORKING_DIR"/bambam-gtfs-flex.toml \
    --flex-directory "$WORKING_DIR" \
    --start-time '2024-09-03T06:00:00' \
    --graph-edge-list 2 \
    --map-edge-list 2 \
    --gtfs-flex-edge-list 2

# 5. write the grid region in the expected query format
